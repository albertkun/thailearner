// Build-time audio generation for the Japanese trainer.
//
// Pre-synthesizes every built-in item (hiragana, katakana, starter words) with
// Microsoft Edge's free neural Japanese voices and writes mp3s into
//   assets/audio/nanami/<id>.mp3   (ja-JP-NanamiNeural, female)
//   assets/audio/keita/<id>.mp3    (ja-JP-KeitaNeural, male)
//
...
for (const [dir, voiceName] of Object.entries(VOICES)) {
  ...
  for (const item of items) {
    ...
    if (buf.length < 800) throw new Error(`Suspiciously small audio for ${item.id} ("${text}") — aborting.`);
